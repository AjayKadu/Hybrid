...
function Dashboard() {
    ...
    function getData() {
        ...
        var myHeaders = {Authorization: getToken};

               axios.get(`http://localhost:9090/employee`, {headers: myHeaders})
                    .then((result)=>{
    ...
    function Add() {
        var getToken = sessionStorage.getItem("token");
        var myHeaders = {Authorization: getToken};
        axios.post(`http://localhost:9090/employee`, employee,{headers: myHeaders})
              .then((result)=>{
    ...
    function Update() {
        var getToken = sessionStorage.getItem("token");
        var myHeaders = {Authorization: getToken};
        axios.put(`http://localhost:9090/employee/ ${employee.emp_id}`, employee,{headers: myHeaders})
              .then((result)=>{
                if (result.data.affectedRows > 0) {
                    getData();
                    clearBoxes();
    ...
    function Delete(No) {
        var getToken = sessionStorage.getItem("token");
        var myHeaders = {Authorization: getToken};
        axios.delete(`http://localhost:9090/employee/ ${No}`,{headers: myHeaders})
             .then((result)=>{
                if (result.data.affectedRows > 0) {
                    getData();
                    clearBoxes();
